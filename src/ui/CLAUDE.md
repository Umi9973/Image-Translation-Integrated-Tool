...
| `workspace.ts` | Workspace controller; exports `renderWorkspace(container, page)`; owns all mutable state (bubbles, sortedIds, selectedId, inpaintPatches); wires Detect / **+ Add Box** / OCR All / Translate All / Copy Prompt / Paste Response / Inpaint All / **Revert All Inpaint** / Typeset All / Download buttons; bubble delete (× button per list item); drag-to-move + 8-handle resize on selected bubble. **+ Add Box** places a new `MangaBubble` (source=`'manual'`, cover=true) at center; shape dropdown (Rect/Bubble) sets the cover shape. Editor panel "Cover background" toggle + shape selector + "Outline" checkbox control the typeset cover fill. Typeset All shows `⚠ dots clipped` amber badge on any bubble list item whose dot run was silently truncated. Download composites original image + inpaint canvas + `renderTypesetToCanvas` onto an offscreen canvas and saves as PNG. Passes `getGlossary()` to `buildPrompt`/`translatePage`. **Per-bubble inpaint revert**: before each inpaint run, canvas region per bubble saved as `ImageData` in `inpaintPatches`; editor shows "Revert Inpaint" button only after inpainting; "Revert All Inpaint" clears entire canvas and patches map. **Inpaint shape-aware**: `shape` and `source` sent to inpaint worker; worker paints an ellipse (not rectangle) when `shape === 'bubble'` so the white cover follows the bubble outline. **Per-bubble typeset revert**: before each typeset run, SVG content per bubble saved in `typesetPatches Map<id, string>`; editor shows "Revert Typeset" button only after typesetted; "Revert All Typeset" clears all typeset SVG elements and patches map. **Typeset All button**: re-evaluated on every `translated_zh` edit so manually-typed text immediately enables the button. |
| `workspace.css` | Workspace layout + component styles; uses CSS vars from `app.css` `:root` |
| `dict-panel.ts` | Left sidebar dictionary panel; exports `renderDictPanel(container)`, `getGlossary(): GlossaryEntry[]`; persists glossary to `localStorage` (`mangavibe_glossary`). **No-Split tab**: session-only phrases injected into BudouX model clone via `addPhraseToParser()` — BudouX won't split them across columns; "Reset all" restores default model. **Glossary tab**: JP→ZH term overrides injected into LLM prompt via `## Glossary` section (applied at translate/copy-prompt time). |
